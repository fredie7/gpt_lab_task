...
def load_documents():
    ...
    # Check each column for nulls and print the result
    ...
    # Normalize symptom column by getting rid of whitespace, if any, and converting to lowercase
    df['symptom'] = df['symptom'].str.strip().str.lower()
    # Rid conditions column of whitespaces, if any, and convert to lowercase
    df['conditions'] = df['conditions'].apply(lambda x: [c.strip().lower() for c in x.split(',')])
    # Rid follow-up questions column of whitespaces, if any, and convert to lowercase
    df['follow_up_questions'] = df['follow_up_questions'].apply(lambda x: [q.strip().lower() for q in x.split(';')])

    # Convert each row into a Document object to help the LLM understand query context
    docs = [
        Document(

...
# @app.post("/ask")
# async def chat(symptom_input: SymptomInput):
#     user_input = symptom_input.message.strip()
#     if not user_input:
...
#     # Run LangGraph
#     result = result_graph.invoke({"messages": messages})
...
async def chat(symptom_input: SymptomInput):
    user_input = symptom_input.message.strip()
    history = symptom_input.history or []

    if not user_input:
        return {"response": "⚠️ No input provided.", "history": history}

    # Append new user message to history
    history.append({"role": "user", "content": user_input})

    # Pass the entire history to the graph
    result = result_graph.invoke({"messages": history})
    
    last_message = result['messages'][-1]
    history.append({"role": "assistant", "content": last_message.content})

    return {"response": last_message.content, "history": history}
